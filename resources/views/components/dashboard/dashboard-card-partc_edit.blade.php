<div>
  {!! Form::open(['action' => ['App\Http\Controllers\PartcController@partc_update', $partc->id], 'method' => 'POST', 'enctype' => 'multipart/form-data' ]) !!}
              ...
              <div class="bg-white px-4 py-5 sm:p-6">
                <div class="grid grid-cols-6 gap-6">
                  ...
                  <div class="col-span-6 sm:col-span-3">
                    ...
                    <select id="id_batch" name="id_batch" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                      @if(count($batch_drop) > 0)
                        @foreach($batch_drop as $shopping)
                            <option value="{{ $shopping->id }}" class="overflow-y-auto py-1 h-48 text-gray-700 dark:text-gray-200" {{$shopping->id  ? 'selected' : '' }}>{{ $shopping->name}}</option>
                        @endforeach
                  ...
                  <div class="col-span-6 sm:col-span-3">
                      ...
                      {{Form::text('name', $partc->name, ['class' => 'mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm'])}}
                  </div>

                  <div class="col-span-6 sm:col-span-3">
                    {{Form::label('ttl', 'Tempat, Tanggal Lahir', ['class' => 'block text-sm font-medium text-gray-700'])}}
                    {{Form::text('ttl', $partc->ttl, ['class' => 'mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm'])}}
                  </div>

                  <div class="col-span-6 sm:col-span-3">
                    {{Form::label('nik', 'Nomor Induk Kependudukan', ['class' => 'block text-sm font-medium text-gray-700'])}}
                    {{Form::text('nik', $partc->nik, ['class' => 'mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm'])}}
                  </div>

                  <div class="col-span-6 sm:col-span-3">
                    {{Form::label('pddk', 'Pendidikan Terakhir', ['class' => 'block text-sm font-medium text-gray-700'])}}
                    <select id="pddk" name="pddk" class="mt-1 block w-full rounded-md border border-gray-300 bg-white py-2 px-3 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm">
                        <option>D3</option>
                        <option>D4</option>
                        <option>S1</option>
                        <option>S2</option>
                  ...
                  <div class="col-span-6 sm:col-span-3">
                    ...
                    <select id="id_itn" name="id_itn" class="mt-1 block w-full rounded-md border border-gray-300 bg-white py-2 px-3 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm">
                      @if(count($itn_drop) > 0)
                        @foreach($itn_drop as $itn)
                            <option value="{{ $itn->id }}" {{$itn->id  ? 'selected' : ''}}>{{ $itn->name}} - {{ $itn->itn_sektor}}</option>
                        @endforeach
                      @else
                            <p>No Instansi</P>
                        @endif
                  ...
                  <div class="col-span-6 sm:col-span-3">
                    ...
                    {{Form::text('notel', $partc->notel, ['class' => 'mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm'])}}
                  </div>

                  <div class="col-span-6 sm:col-span-3">
                    {{Form::label('email', 'E-Mail', ['class' => 'block text-sm font-medium text-gray-700'])}}
                    {{Form::text('email', $partc->email, ['class' => 'mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm'])}}
                  </div>

                  <div class="col-span-6 sm:col-span-3">
                    {{Form::label('size', 'Ukuran Baju PDL Safety', ['class' => 'block text-sm font-medium text-gray-700'])}}
                    <select id="size" name="size" class="mt-1 block w-full rounded-md border border-gray-300 bg-white py-2 px-3 shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm">
                        <option>S</option>
                        <option>M</option>
                        <option>L</option>
                        <option>XL</option>
                  ...
                  <div class="col-span-6 sm:col-span-3">
                    ...
                    {{Form::text('addr', $partc->addr, ['class' => 'mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm'])}}
                  </div>
              ...
              <div class="shadow sm:overflow-hidden sm:rounded-md">
                  <div class="space-y-6 bg-white px-4 py-5 sm:p-6">
                    ...
                    <div class="col-span-6 sm:col-span-3">
                      ...
                      <p class="text-sm text-gray-500 dark:text-gray-300" id="file_input_help">SVG, PNG, JPG (MAX. 800x400px).</p>
                    </div>
                    ...
                    <div class="col-span-6 sm:col-span-3">
                      ...
                      <p class="text-sm text-gray-500 dark:text-gray-300" id="file_input_help">SVG, PNG, JPG (MAX. 10MB).</p>
                    </div>
    ...
    <div class="bg-gray-50 px-4 py-3 text-right sm:px-6">
      {{form::submit('Create', ['class' => 'inline-flex justify-center rounded-md border border-transparent bg-indigo-600 py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2', 'name' => 'batch_input'])}}
    </div>
